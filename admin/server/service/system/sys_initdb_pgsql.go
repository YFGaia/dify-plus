...
// GetJwtSigningKeyFormDifyApiEnv 从dify api的.env文件中获取jwt的加密key
func (h PgsqlInitHandler) GetJwtSigningKeyFormDifyApiEnv() (secretKey string, err error) {
	apiEnvPath := filepath.Join("../../", "api", ".env")
	viperAPI := viper.New()
	viperAPI.SetConfigFile(apiEnvPath)
	viperAPI.SetConfigType("env")
	if err = viperAPI.ReadInConfig(); err != nil {
		err = fmt.Errorf("Failed to read .env file: %s\n", err)
		return
	}
	secretKey = viperAPI.GetString("SECRET_KEY")
	return
}
